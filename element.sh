...
if [[ $1 ]]
  ...
  then
    ...
    echo $ELEMENT | while IFS=\| read ATOM_NUM ATOM_MASS MELT_P BOIL_P SYMBOL NAME TYPE
    do
      echo "The element with atomic number $ATOM_NUM is $NAME ($SYMBOL). It's a $TYPE, with a mass of $ATOM_MASS amu. $NAME has a melting point of $MELT_P celsius and a boiling point of $BOIL_P celsius."
    done
  fi
  else
